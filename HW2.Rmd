...
The proportion of consumers who buys from firm i is given by:
...
$$
D_i(p_i, p_j) = Pr(s_i \ge s_j)L
$$

$$
D_i(p_i, p_j) = L \int\space F[x_i - (p_i - p_j) - L(p_i,p^e) + L(p_j,p^e)]f(x_i)dx_i
$$
...
Substituting the equilibrium value $p_i$ = $p_j$ we obtain

$$
\frac{\delta D_i}{\delta p_i} = - L \int\space f(x_i)^2dx_i
$$
