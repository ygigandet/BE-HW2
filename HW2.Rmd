...
$$
The profit for firm i is given by:

$$
\pi_i(p_i, p_{-i}) = (p_i - c)D_i(p_i,p_{-i})
$$

The equilibrium price must satisfy the following condition:
...
$$
As we assume that all the quantities are the same, $q$, thus:

$$
p^* = \arg \max_{p_{i}} (p_i - c) \frac{e^\frac{q - p_i}{\sigma}}{\sum_{j = 1}^{n}e^\frac{q - p_j}{\sigma}}
$$
Now we assume that in a symmetric equilibrium every firm will choose the same price $p^*$

$$
p^* = \arg \max_{p^*} (p^* - c) \frac{e^\frac{q - p^*}{\sigma}}{n * e^\frac{q - p^*}{\sigma}}
$$

With the hint, maximizing the profit is the same as maximizing ln of the profit:

$$
p^* = (p^* - c) \frac{\frac{q - p^*}{\sigma}}{n *\frac{q - p^*}{\sigma}}
$$
$$
p^* * (n *\frac{q - p^*}{\sigma}) = (p^* - c) \frac{q - p^*}{\sigma}
$$

$$
\frac{q - p^*}{\sigma}(p^*n - p^* + c) = 0
$$

$$
\frac{q - p^*}{\sigma}c + p^* \frac{q - p^*}{\sigma}(n - 1) = 0
$$
...
If the social degree of complexity were to be equal to 0, this mean that the firm will be fully transparent over its based and add-on price. In the case of our printer sales, a naive consumer might decided to go to another firm with lower based price and higher(but avoidable) add-on.

b)
The demand for firm i is:
$$D_{i} = P(q_{i}-p_{i} + \varepsilon_{i} \ge \displaystyle \max_{j\neq i}q_{j} - p_{j}+\varepsilon_{j})$$
if we imagine an price equilibrium such that
$p_{j} = p^*, \forall j\neq i$
Then the profit with a sophisticated consumer is:
$$\pi = (p+\hat{p})D(-p+p^*)$$
